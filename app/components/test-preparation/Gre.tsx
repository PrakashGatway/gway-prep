...
function GreSection({ examFormatSection, whatIsGreSection, cta_banner, slug }: any) {
  ...
  return (
    <section className="w-full max-w-7xl mx-auto px-4 py-8 md:py-12 text-[#2d2d2d] bg-white">
      {/* Top Banner */}
      <div className="w-full bg-[#f06437] text-white rounded-2xl md:rounded-[30px] px-6 md:px-12 lg:px-20 py-6 md:py-8 flex
...
function GrePatternTable({ examPatternData }: { examPatternData: any }) {
  ...
  if (!examPatternData || !examPatternData['exam-patternTable'] || examPatternData['exam-patternTable'].length === 0) {
    return null;
  }
                    ...
                    {patternTableRows.map((row: any, rowIdx: number) => (
                      ...
                      >
                        <td 
                          ...
                          dangerouslySetInnerHTML={{ __html: row.section || "" }}
                        />
          ...
          border-2 border-[#D9D9D9] bg-[#FAFAFA] px-6 md:px-10 lg:px-16 py-10 md:py-14 lg:py-16">
            <div className="max-w-2xl text-center lg:text-left">
              ...
              <p className="mt-6 md:mt-8 text-[#5F5F5F] text-base md:text-lg lg:text-[1.2rem] leading-relaxed">
                {videoData?.Subtitle || "Want to go further? We also include 290+ curated lessons shaped by student feedback to deliver material in the way you learn best."}
              </p>
  ...
  const resources = [
    {
      ...
      description: resourcesData?.resources?.[0]?.description || "Take the GRE practice material and begin your GRE preparation now.",
      button: resourcesData?.resources?.[0]?.buttonText || "Take GRE Practice Material",
      filled: true,
    },
    {
      title: resourcesData?.resources?.[1]?.title || "--",
      description: resourcesData?.resources?.[1]?.description || "Begin your GRE coaching with resources prepared by our experts to help you with your GRE prep.",
      button: resourcesData?.resources?.[1]?.buttonText || "Download GRE Other Resources",
      filled: false,
    },
    {
      title: resourcesData?.resources?.[2]?.title || "--",
      description: resourcesData?.resources?.[2]?.description || "Download the GRE syllabus now and get a head start on your GRE preparation.",
      button: resourcesData?.resources?.[2]?.buttonText || "Download GRE Syllabus",
      filled: true,
    },
  ];
    ...
    <section className="py-12 md:py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-8 md:mb-14">
          <div
            dangerouslySetInnerHTML={{
              __html: resourcesData?.title || "<h2 className='text-3xl md:text-4xl lg:text-5xl font-semibold'><span className='text-[#222]'>Free GRE </span><span className='text-[#F5632A]'>Prep Resources</span></h2>"
            }}
          />
        </div>
