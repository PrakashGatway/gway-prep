...
const GreForm: React.FC<GreFormProps> = ({ studentsData }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setActiveIndex((prev) => (prev + 1) % studentsData.length);
    }, 3500);
                ...
                <div className="flex-1 min-h-0">
                  <AnimatePresence mode="wait">
                    {studentsData?.length > 0 ? (
                      ...
                      >
                        {(() => {
                          const ele = studentsData[activeIndex];

                          return (
                            <div className="border-4 border-[#F7EED8] p-3 sm:p-4 lg:p-5 h-full flex flex-col rounded-xl">
                              {/* Quote */}
                              <div className="text-[#F7C76A] text-2xl sm:text-3xl lg:text-4xl leading-none">
                                “
                              </div>
                              ...
                              <div className="mt-3 sm:mt-4 flex items-center gap-2 flex-wrap">
                                ...
                                <span className="uppercase tracking-[0.2em] text-[8px] sm:text-[9px] md:text-[10px] text-gray-600">
                                  {ele.course} TOTAL
                                </span>
                              </div>
                              ...
                              <div className="mt-3 sm:mt-4 flex items-center gap-3">
                                <div className="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 rounded-full bg-[#FFF5D8] flex items-center justify-center font-bold text-[#7B5B00] text-sm sm:text-base lg:text-lg flex-shrink-0">
                                  {ele.name
                                    ?.split(" ")
                                    ?.map((n: string) => n[0])
                                    ?.join("")
